...
class record:
    def __init__(self, oligos=dict(), sequences=dict(), folder=None):
        ...
        Oligos and sequences dicts can be provided already.
        """
        self.oligos = oligos
    ...
    def fragmentize(self, name):
        """Figure out if a parameter is a fragment name or a raw fragment
        variable, and returns the fragment."""
    ...
    def show_map(self, name, zoom=None, topology=None, width=config.default_plot_width):
        ...
        if len(graphic_record.features) >= config.max_features:
            print(
                f"Too many features ({len(sequence.features)}) to be plotted. Try cropping or increasing the maximum."
            )
            return
    ...
    def show_sequence(
        ...
        if translate:
            ...
            graphic_record.plot_translation(
                ax, translate, fontdict={"weight": "bold"}, long_form_translation=False
            )

    #### I/O
    def find_gb(self, filename, folder=None, extension=".gb", name=None):
        """Find a gb file that matches the name (with .gb extension), and
        import it as a sequence."""
        if not folder:
            folder = self.folder
    ...
    def write_gb(self, name, filename=None, folder=None):
        ...
        if type(fragment) in [pydna.contig.Contig, pydna.dseqrecord.Dseqrecord]:
            if fragment.locus == "name":
                fragment.locus = (
                    filename if filename else name
                )  # Set the locus variable to the fragment's name
    ...
    def clean_features(
        ...
        for i in feats:
            ...
            if not "label" in i.qualifiers.keys():
                if verbose:
                    print(f'Removing feature "{i}" that has no label.')
                keep = False
            elif i.qualifiers["label"][0] in junk:
                if verbose:
                    print(f'Removing junk feature "{i.qualifiers["label"][0]}".')
                keep = False
            elif i.type in junk_types:
                if verbose:
                    print(f'Removing primer {i.qualifiers["label"][0]}')
                keep = False
    ...
    def new_feature(self, sequence, location, strand, name, feature_type, force=False):
        ...
        feature_location = Bio.SeqFeature.FeatureLocation(
            location[0], location[1], strand
        )
    ...
    def find_features(self, name, targets, margins=(0, 0), plot=True):
        ...
        margins is a tuple giving how many base pairs to include before
        and after. Return a tuple of coordinates.
        """
        sequence = self.sequences[name] if type(name) == str else name
    ...
    def find_motif(self, target, motif, strand):
        ...
        if strand == -1:
            motif = Bio.Seq.Seq(motif).reverse_complement()
    ...
    def find_orfs(self, target, strand, min_length=300, start_codon="ATG", greedy=True):
        """Detect ORFs in the strand.

        Returns the first and last nucleotides' coordinates. Greedy mode
        will only return the longest ORF associated with each stop
        codon.
        """
        sequence = self.sequences[target] if type(target) == str else target
        ...
        if greedy:
            ...
            longest = {
                stop: max([i[1] - i[0] for i in orfs if i[1] == stop]) for stop in stops
            }
    ...
    def find_primers(
        self, target, oligos=None, plot=True, zoom=None, lim=config.min_primer_length
    ):
        """Find primers that bind on a given target sequence.

        If oligos are not specified, use the record's oligo dict.
        Otherwise, oligos can be provided as a dict, a list/tuple, or a
        single oligo.
        """
        sequence = self.sequences[target] if type(target) == str else target
        ...
        # Find binding sites
        anneal = pydna.all.Anneal(template=sequence, primers=oligos_pydna, limit=lim)
        ...
        if plot:
            ...
            for i in anneal.forward_primers:
                gr.features.append(
                    gf(
                        start=i.position,
                        end=i.position - len(i.footprint),
                        strand=-1,
                        color="#90bc21",
                        label=i.name,
                    )
            ...
            for i in anneal.reverse_primers:
                gr.features.append(
                    gf(
                        start=i.position + len(i.footprint),
                        end=i.position,
                        strand=+1,
                        color="#c65423",
                        label=i.name,
    ...
    def pcr(
        ...
        self.clean_features(
            amplicon, junk=[], junk_types=["primer_bind"], verbose=False
        )
    ...
    def pair(self, o1, o2):
        """Anneal together a pair of oligos, either called by their name or
        directly input as a sequence."""
        if type(o1) == str:
    ...
    def point_mutation(self, template, mutation, name=None):
        """Introduces a point mutation in a sequence.

        Input is a string: <before><position><after>.
        """
        ...
        sequence_mutated = pydna.all.Dseqrecord(
            record=mutated.toseq(), circular=sequence.circular
        )
    ...
    def check_homology(self, s1, s2, minimum=10, maximum=100, show_linker=True):
        """Check that two sequences have homology regions suitable for Gibson
        assembly."""
    ...
    def ligate(self, fragments, name=None):
        ...
        Fragments can be either sequences or names.
        """
        ...
        while True:
            ...
            if last in five:  # Move on to the next fragment
                ...
                break
        print(
            f'Fragments assembled in this order: {graph}. ({"circular" if circular else "linear"})'
        )
    ...
    def make_primers(
        ...
        lim=18,
        plot=True,
        plot_annealing=False,
        print_tm=False,
    ):
        """Creates a pair of primers to amplify the region `location` from
        target."""
        sequence = self.sequences[target] if type(target) == str else target
        ...
        if plot_annealing:
            self.show_sequence(
                sequence,
                (max(0, start - 20), start + len(r) + 20),
                highlight=(start, start + len(f)),
            )
            self.show_sequence(
                sequence,
                (end - len(r) - 20, min(len(sequence), end + 20)),
    ...
    def make_junction(self, left, right, linker=None, plot=True, homology=40):
        """Design tailed oligos for homology-based assembly.

        Input is 1) the reverse primer of the first fragment, 2) the
        forward primer of the second fragment, 3) optionally a linker to
        ...
        bottom = (
            Bio.Seq.reverse_complement(right).lower()
            + Bio.Seq.reverse_complement(linker).lower()
            + left.upper()
        )
    ...
    def latest(self, prefix=None):
        """Looks at already existing oligos in the form `prefix+integer`, and
        finds what the next one should be."""
        if not prefix:
            prefix = config.prefix

        indices = [
            int(i[len(prefix) :])
            for i in self.oligos.keys()
            if i[0 : len(prefix)] == prefix
        ]
        return prefix + str(max(indices) + 1)

    def new_oligos(self, oligos, prefix=None):
        """Determines if each oligo from the list is new.

        If it is, finds a name for it in the form `prefix+integer`.
        """
        if not prefix:
            prefix = self.prefix
