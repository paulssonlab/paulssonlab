"""Example app demonstrating how to use the HoloViews API to generate a bokeh
app with complex interactivity.

Uses a RangeXY stream to allow interactive exploration of the mandelbrot
set.
"""
...
def mandel(x, y, max_iters):
    ...
    is a candidate for membership in the Mandelbrot set given a fixed number of
    iterations."""
    i = 0
    c = complex(x, y)
    z = 0.0j
...
def get_fractal(x_range, y_range):
    ...
    return hv.Image(
        create_fractal(x0, x1, -y1, -y0, image, 200), bounds=(x0, y0, x1, y1)
    )
...
dmap = hv.DynamicMap(
    get_fractal, label="Manderbrot Explorer", streams=[range_stream]
).hist(log=True)
