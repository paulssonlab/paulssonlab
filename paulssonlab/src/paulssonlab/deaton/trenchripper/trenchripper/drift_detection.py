...
def get_orb_pois(
    im1, im2, max_interest_points=750, match_threshold=0.1, plot=False, pad_width=15
):
    ...
    if plot:
        im3 = cv2.drawMatches(
            im1_scaled, keypoints1, im2_scaled, keypoints2, matches, None
        )
...
def find_template(
    reference, check, match_threshold=0.1, window_height=400, window_width=200
):
    ...
    # Get interest points
    points1, points2, _ = get_orb_pois(
        reference_scaled, check_scaled, match_threshold=match_threshold
    )
    ...
    top_left = np.array(
        [
            max(0, int(median_position[0] - window_width / 2)),
            max(0, int(median_position[1] - window_height / 2)),
        ]
    )
    # Crop image to get template
    template = reference_scaled[
        top_left[1] : top_left[1] + window_height,
        top_left[0] : top_left[0] + window_width,
    ]
