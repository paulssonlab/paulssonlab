...
class kymograph_interactive(kymograph_multifov):
    ...
    def preview_midpoints_interactive(self):
        midpoint_drift = interactive(
            self.preview_midpoints,
            {"manual": True},
            smoothed_x_percentiles_list=fixed(self.smoothed_x_percentiles_list),
...
class fluo_segmentation_interactive(fluo_segmentation):
    ...
    def import_array(
        ...
        selecteddf = self.kymodf.loc[
            list(zip(rand_trench_arr, np.zeros(len(rand_trench_arr)).astype(int)))
        ]
        selectedlist = list(
            zip(
                selecteddf["File Index"].tolist(),
                selecteddf["File Trench Index"].tolist(),
            )
        )
    ...
    def plot_eigval(self, proc_list):
        ...
        for proc in proc_list:
            ...
            min_eigvals = np.array(
                [
                    self.hessian_contrast_enc(inverted[t], self.hess_pad)
    ...
    def plot_threshold_result(
        self, eigval_list, cell_mask_list, edge_threshold_scaling, min_obj_size
    ):
        composite_mask_list = []
        edge_mask_list = []
