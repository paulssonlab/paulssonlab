...
class kychunker(timechunker):
    ...
    def get_smoothed_y_percentiles(
        ...
        """
        y_percentiles_path = self.chunk_t(
            (imported_hdf5_handle[self.seg_channel],),
            (2,),
            1,
        ...
        with h5py_cache.File(
            y_percentiles_path, "r", chunk_cache_mem_size=self.chunk_cache_mem_size
        ) as y_percentiles_handle:
            y_percentiles_smoothed = self.median_filter_2d(
                y_percentiles_handle["data"], smoothing_kernel_y
            )
    ...
    def get_trench_edges_y(
        ...
        trench_mask_y = self.triangle_threshold(
            y_percentiles_smoothed_array, triangle_nbins, triangle_scaling
        )
    ...
    def get_phase_orientations(
        self, y_percentiles_smoothed, valid_edges_y_list, pad=50, percentile=90
    ):
        ...
        for row in range(valid_edges_y_list[0].shape[0] // 2):
            edge_1, edge_2 = (
                valid_edges_y_list[0][2 * row],
                valid_edges_y_list[0][2 * row + 1],
            )
            edge_1_val = np.percentile(
                y_percentiles_smoothed[:, 0][edge_1 : edge_1 + pad], percentile
            )
            edge_2_val = np.percentile(
                y_percentiles_smoothed[:, 0][edge_2 - pad : edge_2], percentile
            )
    ...
    def keep_in_frame_kernels(self, trench_edges_y_list, y_drift, max_y_dim, padding_y):
        ...
        edge_under_max = np.all(
            (trench_edges_y_list + max_drift + padding_y) < max_y_dim, axis=0
        )
        edge_over_min = np.all(
            (trench_edges_y_list + min_drift - padding_y) >= 0, axis=0
        )
        ...
        valid_edges_y_list = [
            trench_edges_y[valid_edge_mask] for trench_edges_y in trench_edges_y_list
        ]
        ...
        for t in range(imported_hdf5_array.shape[2]):
            ...
            for r, orientation in enumerate(trench_orientations):
                if orientation == 0:
                    ...
                    lower = min(
                        trench_edge_y + trench_len_y, imported_hdf5_array.shape[0]
                    )
                    pad = upper + trench_len_y + padding_y - lower
                    output_array = np.pad(
                        imported_hdf5_array[upper:lower, :, t],
                        ((pad, 0), (0, 0)),
                        "constant",
                    )
    ...
    def crop_trenches_in_y(self):
        ...
        with h5py_cache.File(
            ...
            y_percentiles_smoothed = self.get_smoothed_y_percentiles(
        ...
        if self.orientation_detection == "phase":
            with h5py_cache.File(
                ...
                valid_edges_y_list, _ = self.keep_in_frame_kernels(
                    trench_edges_y_list,
                    y_drift,
                    imported_hdf5_handle[self.seg_channel].shape[0],
                    self.padding_y,
                )
            self.trench_orientations = self.get_phase_orientations(
                y_percentiles_smoothed, valid_edges_y_list
            )

        elif self.orientation_detection == 0 or self.orientation_detection == 1:
            self.trench_orientations = self.get_manual_orientations(
                trench_edges_y_list,
            ...
            with h5py_cache.File(
                ...
                valid_edges_y_list, valid_orientation_mask = self.keep_in_frame_kernels(
                    trench_edges_y_list,
                    y_drift,
                    imported_hdf5_handle[self.seg_channel].shape[0],
                    self.padding_y,
                )
            self.trench_orientations = np.array(self.trench_orientations)[
                valid_orientation_mask
            ].tolist()
        ...
        with h5py_cache.File(
            ...
            cropped_in_y_path = self.chunk_t(
                (imported_hdf5_handle[self.seg_channel], y_drift),
                (2, 0),
                3,
                self.crop_y,
                "cropped_in_y_" + str(self.seg_channel),
                "data",
                valid_edges_y_list[0],
                self.padding_y,
                self.trench_len_y,
                self.trench_orientations,
                t_range_tuple=(self.t_range, (0, None)),
                write_coords=True,
            )
        ...
        for channel in self.all_channels[1:]:
            with h5py_cache.File(
                ...
                cropped_in_y_path = self.chunk_t(
                    (imported_hdf5_handle[channel], y_drift),
                    (2, 0),
                    3,
                    self.crop_y,
                    "cropped_in_y_" + str(channel),
                    "data",
                    valid_edges_y_list[0],
                    self.padding_y,
                    self.trench_len_y,
    ...
    def get_k_masks(
        ...
        stays_in_frame = np.all(midpoints_up >= 0, axis=0) * np.all(
            midpoints_dn <= cropped_in_y.shape[2], axis=0
    ...
    def crop_with_k_masks(self, cropped_in_y_paths, k_mask_handle, row_num, k_tot):
        ...
        for c, channel in enumerate(self.all_channels):
            ...
            with h5py_cache.File(
                cropped_in_y_paths[c],
                "r",
                chunk_cache_mem_size=self.chunk_cache_mem_size,
            ) as cropped_in_y_handle:
                kymograph_path = self.chunk_t(
    ...
    def get_crop_in_x(
        ...
        for row_num, all_midpoints in enumerate(all_midpoints_list):
            ...
            with h5py_cache.File(
                cropped_in_y_paths[0],
                "r",
                chunk_cache_mem_size=self.chunk_cache_mem_size,
            ) as cropped_in_y_handle:
                k_mask_path, k_tot = self.get_k_masks(
            ...
            with h5py_cache.File(
                k_mask_path, "r", chunk_cache_mem_size=self.chunk_cache_mem_size
            ) as k_mask_handle:
                self.crop_with_k_masks(
                    cropped_in_y_paths, k_mask_handle, row_num, k_tot
                )
    ...
    def crop_trenches_in_x(self, cropped_in_y_paths):
        ...
        """
        with h5py_cache.File(
            cropped_in_y_paths[0], "r", chunk_cache_mem_size=self.chunk_cache_mem_size
        ) as cropped_in_y_handle:
            smoothed_x_percentiles_path = self.chunk_t(
                (cropped_in_y_handle["data"],),
                (3,),
                2,
        ...
        with h5py_cache.File(
            smoothed_x_percentiles_path,
            "r",
            chunk_cache_mem_size=self.chunk_cache_mem_size,
        ) as smoothed_x_percentiles_handle:
            all_midpoints_list = self.get_all_midpoints(
                smoothed_x_percentiles_handle["data"],
                self.otsu_nbins,
                self.otsu_scaling,
            )
    ...
    def save_coords(self):
        ...
        for l, x_coord in enumerate(self.x_coords):
            ...
            for k in range(scaled_x_coord.shape[0]):
                ...
                pd_output.append(
                    np.array(
                        [fs, ls, ks, tpts, ts, orit, yt, xt, global_yt, global_xt]
                    ).T
                )
    ...
    def collect_metadata(self, fov_list, use_archive=False, overwrite_archive=True):
        ...
        idx_df = (
            df_out.groupby(["fov", "lane", "trench"])
            .size()
            .reset_index()
            .drop(0, axis=1)
            .reset_index()
        ...
        idx_df = idx_df.set_index(
            ["fov", "lane", "trench"], drop=True, append=False, inplace=False
        )
        idx_df = idx_df.reindex(labels=df_out.index)
        df_out["trenchid"] = idx_df["index"]

        meta_out_handle = pandas_hdf5_handler(self.metapath)
        meta_out_handle.write_df(
            "kymo", df_out, metadata={"attempted_fov_list": fov_list}
        )
    ...
    def reorg_kymographs(self, fov_number):
        ...
        proc_file_path = (
            self.output_path + "/kymo_proc_" + str(self.fov_number) + ".hdf5"
        )
        ...
        with h5py.File(self.output_file_path, "r") as infile:
            with h5py.File(proc_file_path, "w") as outfile:
                ...
                for lane in lane_list:
                    ...
                    trench_list = (
                        lane_handle.index.get_level_values("trench").unique().tolist()
                    )
    ...
    def dask_full_kymograph(self, dask_controller, fov_list=None):
        ...
        successful_fovs_future = dask_controller.daskclient.submit(
            collectmeta, kymo_futures_list, retries=0
        )
        ...
        for reorg_idx, fov in enumerate(fov_list):
            future = dask_controller.daskclient.submit(
                reorgkymo, successful_fovs_future, fov, retries=0
            )
            dask_controller.futures["reorg_kymographs: " + str(reorg_idx)] = future
            reorg_futures.append(future)

        success_future = dask_controller.daskclient.submit(
            cleankymo, reorg_futures, retries=0
        )
...
class kymograph_multifov(multifov):
    ...
    def import_hdf5(self, i):
        ...
        if self.t_range[1] == None:
            ...
            t_len = list(range(hdf5_handle[self.seg_channel].shape[2]))[
                self.t_range[1] - 1
            ]
        ...
        for channel in self.all_channels:
            arr_list.append(
                np.concatenate(
                    [
                        hdf5_handle[channel][:, :, idx][:, :, np.newaxis]
                        for idx in indices
                    ],
                    axis=2,
                )
    ...
    def get_phase_orientations(
        self, i, y_percentiles_smoothed_list, valid_edges_y_lists, pad=50, percentile=90
    ):
        """Automatically determines the orientations of trench rows when segmenting with phase. Only
        considers the first timepoint. Currently the only mechanism to do this, until a manual version
        is implemented.
    ...
    def keep_in_frame_kernels(
        self, i, trench_edges_y_lists, y_drift_list, imported_array_list, padding_y
    ):
        """Removes those kernels which drift out of the image during any timepoint.
        Args:
        ...
        for t in range(imported_array.shape[3]):
            ...
            for r, orientation in enumerate(trench_orientations):
                ...
                for c in range(imported_array.shape[0]):
                    output_array = np.pad(
                        imported_array[c, upper:lower, :, t],
                        ((pad, 0), (0, 0)),
                        "constant",
                    )
