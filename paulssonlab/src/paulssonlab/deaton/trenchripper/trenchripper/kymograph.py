...
class kychunker(timechunker):
    ...
    def get_init_phase_orientations(
        self, y_percentiles_smoothed_handle, trench_edges_y_list, pad=50, percentile=90
    ):
        ...
        for row in range(trench_edges_y_list[0].shape[0] // 2):
            edge_1, edge_2 = (
                trench_edges_y_list[0][2 * row],
                trench_edges_y_list[0][2 * row + 1],
            )
    ...
    def crop_trenches_in_y(self, imported_hdf5_handle):
        ...
        trench_orientations = self.get_init_phase_orientations(
