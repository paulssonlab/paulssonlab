...
class hdf5_fov_extractor:
    def __init__(
        self, nd2filename, headpath, chunk_shape=(2048, 2048, 1)
...
class hdf5_extractor:
    ...
    def extract_meta(self, i, nd2filename, init_meta=False):
        ...
        if init_meta:
            ...
            df_out = (
                pd.concat([df, fov_metadata]).reset_index(drop=True).sort_values("t")
            )
...
class nd_metadata_handler:
    ...
    def make_fov_df(
        ...
        output = output.astype(
            {"fov": int, "t": float, "x": float, "y": float, "z": float}
        )
