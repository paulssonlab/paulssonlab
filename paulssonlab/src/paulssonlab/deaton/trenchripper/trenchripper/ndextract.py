...
class hdf5_fov_extractor:
    def __init__(
        self, nd2filename, headpath, tpts_per_file=100, ignore_fovmetadata=False
    ...
    def assignidx(self, expmeta, metadf=None):
        ...
        if metadf is None:
            ...
            timepoint_idx = np.repeat(
                np.array(list(range(timepoints_per_fov)))[np.newaxis, :],
                numfovs,
                axis=0,
            ).flatten()
    ...
    def writemetadata(self, manual_num_frames=None, manual_num_fovs=None):
        ndmeta_handle = nd_metadata_handler(
            self.nd2filename, ignore_fovmetadata=self.ignore_fovmetadata
        )
        if self.ignore_fovmetadata:
            exp_metadata = ndmeta_handle.get_metadata()
        else:
            exp_metadata, fov_metadata = ndmeta_handle.get_metadata(
                manual_num_frames=manual_num_frames, manual_num_fovs=manual_num_fovs
            )
...
class tiff_to_hdf5_extractor:
    ...
    def assignidx(self, metadf):
        ...
        fov_file_idx = np.repeat(
            list(range(files_per_fov)), self.tpts_per_file * numchannels
        )[: -(self.tpts_per_file - remainder) * numchannels]
        file_idx = np.concatenate(
            [fov_file_idx + (fov_idx * files_per_fov) for fov_idx in range(numfovs)]
        )
        ...
        fov_img_idx = np.repeat(
            np.repeat(np.array(list(range(self.tpts_per_file))), numchannels)[
                np.newaxis, :
            ],
            files_per_fov,
            axis=0,
        )
        fov_img_idx = fov_img_idx.flatten()[
            : -(self.tpts_per_file - remainder) * numchannels
        ]
        img_idx = np.concatenate([fov_img_idx for fov_idx in range(numfovs)])
        outdf["File Index"] = file_idx
    ...
    def writemetadata(self, parser, tiff_files, manual_metadata_params={}):
        ...
        for f in tiff_files:
            ...
            if match is not None:
                if first_successful_file:
                    ...
                    chunk_bytes = (
                        2 * np.multiply.accumulate(np.array(self.chunk_shape))[-1]
                    )
                    self.chunk_cache_mem_size = 2 * chunk_bytes
                    (
                        exp_metadata["chunk_shape"],
                        exp_metadata["chunk_cache_mem_size"],
                    ) = (self.chunk_shape, self.chunk_cache_mem_size)
                    # get metadata from the file name
                    fov_metadata = dict(
                        [(key, [value]) for key, value in match.named.items()]
                    )
        ...
        fov_metadata = (
            fov_metadata.set_index(["lane", "fov"])
            .sort_values("timepoints")
            .sort_index()
        )
        # Turn lane and field of view to a single index by mapping pairs of indices to single indices
        old_labels = [
            list(frozen_array) for frozen_array in fov_metadata.index.unique().labels
        ]
        ...
        assignment_metadata = self.assignidx(
            fov_metadata.set_index(["fov", "timepoints"])
        )

        # Get the the image paths corresponding to channel and timepoint for each file index
        channel_paths_by_file_index = assignment_metadata.reset_index()[
            ["File Index", "channel", "Image Path"]
        ].set_index("File Index")
        channel_paths_by_file_index = [
            (
                file_index,
                list(channel_paths_by_file_index.loc[file_index]["channel"]),
                list(channel_paths_by_file_index.loc[file_index]["Image Path"]),
            )
            for file_index in channel_paths_by_file_index.index.unique("File Index")
        ]
    ...
    def extract(self, dask_controller):
        ...
        def writehdf5(fidx_channels_paths):
            ...
            with h5py_cache.File(
                ...
                for i, channel in enumerate(self.metadata["channels"]):
                    hdf5_dataset = h5pyfile.create_dataset(
                        str(channel),
                        (len(filepaths) / num_channels, y_dim, x_dim),
                        chunks=self.chunk_shape,
                        dtype="uint16",
                    )
        ...
        dask_controller.futures["extract file"] = dask_controller.daskclient.map(
            writehdf5, channel_paths_by_file_index
        )
