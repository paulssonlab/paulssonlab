...
class mother_tracker:
    ...
    def get_growth_props(self, file_idx, lower_threshold=0.35, upper_threshold=0.75):
        ...
        file_df = pd.read_hdf(
            os.path.join(self.phasedatapath, "data_%d.h5" % file_idx), "metrics"
        )
        ...
        for trench in trenches:
            ...
            (
                ...
                dt_smoothed_data,
                growth_rate_data,
            ) = self.get_mother_cell_growth_props(
                mother_df,
            ...
            if dt_data is not None:
                ...
                dt_data = np.concatenate(
                    [
                        dt_data,
                        np.array([trench] * dt_data.shape[0])[:, None],
                        np.array([trenchid] * dt_data.shape[0])[:, None],
                        np.array([fov] * dt_data.shape[0])[:, None],
                    ],
                    axis=1,
                )
                dt_smoothed_data = np.concatenate(
                    [
                        dt_smoothed_data,
                        np.array([trench] * dt_smoothed_data.shape[0])[:, None],
                        np.array([trenchid] * dt_smoothed_data.shape[0])[:, None],
                        np.array([fov] * dt_smoothed_data.shape[0])[:, None],
                    ],
                    axis=1,
                )
                growth_rate_data = np.concatenate(
                    [
                        growth_rate_data,
                        np.array([trench] * growth_rate_data.shape[0])[:, None],
                        np.array([trenchid] * growth_rate_data.shape[0])[:, None],
                        np.array([fov] * growth_rate_data.shape[0])[:, None],
                    ],
                    axis=1,
                )
        ...
        file_dt = np.append(
            file_dt, np.array([file_idx] * file_dt.shape[0])[:, None], axis=1
        )
        file_dt_smoothed = np.append(
            file_dt_smoothed,
            np.array([file_idx] * file_dt_smoothed.shape[0])[:, None],
            axis=1,
        )
        file_growth_rate = np.append(
            file_growth_rate,
            np.array([file_idx] * file_growth_rate.shape[0])[:, None],
            axis=1,
        )
        return file_dt, file_dt_smoothed, file_growth_rate

    #     @profile
    def save_all_growth_props(
        self, file_list=None, lower_threshold=0.3, upper_threshold=0.75
    ):
        ...
        for file_idx in file_list:
            file_dt, file_dt_smoothed, file_growth_rate = self.get_growth_props(
                file_idx,
                lower_threshold=lower_threshold,
                upper_threshold=upper_threshold,
            )
        ...
        all_growth_rate = pd.DataFrame(
            ...
            columns=[
                ...
                "igr_area_smoothed",
                "igr_length_normed",
                "igr_length_smoothed_normed",
                "igr_area_normed",
                "igr_area_smoothed_normed",
                "file_trench_idx",
                "trenchid",
                "fov",
                "file_idx",
    ...
    def get_mother_cell_growth_props(
        self, mother_data_frame, lower_threshold=0.35, upper_threshold=0.75
    ):
        ...
        if cutoff_index < 5:
            return None, None, None
        for i in range(
            min(len(loading_fractions) - 4, self.timepoint_full_trenches),
            len(loading_fractions) - 4,
        ):
        ...
        major_axis_length = np.array(mother_data_frame["major_axis_length"])[
            :cutoff_index
        ]
        minor_axis_length = np.array(mother_data_frame["minor_axis_length"])[
            :cutoff_index
        ]
        ...
        repaired_data = self.repair_trench_loadings(
            np.array([major_axis_length, area, minor_axis_length, solidity]).T,
        ...
        growth_rate_data = np.array(
            [
                ...
                instantaneous_growth_rate_area_smoothed,
                minor_axis_length,
                solidity,
            ]
        ).T
    ...
    def repair_trench_loadings(self, data_raw, outside_thresholds):
        ...
        for i in range(window_size, data.shape[0] - window_size):
            ...
            right_higher_than = (
                data[i, :] > data[i + 1 : i + window_size + 1, :] * higher_factor
            )
            left_lower_than = data[i, :] < data[i - window_size : i, :] * lower_factor
            right_lower_than = (
                data[i, :] < data[i + 1 : i + window_size + 1, :] * lower_factor
            )
            if np.any(left_higher_than) * np.any(right_higher_than):
                average_kernel = np.concatenate(
                    (
                        np.any(left_higher_than, axis=1),
                        np.array([0]),
                        np.any(right_higher_than, axis=1),
                    )
                )
                ...
                average_kernel = np.concatenate(
                    (
                        np.any(left_lower_than, axis=1),
                        np.array([0]),
                        np.any(right_lower_than, axis=1),
                    )
                )
                x = np.argwhere(average_kernel)
                data_window = data[i - window_size : i + window_size + 1, :]
