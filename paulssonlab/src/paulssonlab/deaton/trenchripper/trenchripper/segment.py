...
class phase_segmentation:
    ...
    def findWatershedMask(self, img, mask):
        ...
        img_mask = sk.morphology.binary_dilation(
            img_mask, selem == np.ones((1, 3), dtype=np.bool)
        )
...
class phase_segmentation_cluster(phase_segmentation):
    ...
    def dask_segment(self, dask_controller, file_list=None, overwrite=True):
        ...
        if file_list is None:
            file_list = (
                self.meta_handle.read_df("kymograph", read_metadata=True)["File Index"]
                .unique()
                .tolist()
            )
    ...
    def dask_extract_cell_data_mask(
        self, dask_controller, channels, props_to_grab, file_list=None, overwrite=False
    ):
            ...
            for k2, channel in enumerate(channels):
                ...
                if segmented_masks_file in os.listdir(self.phasesegmentationpath):
                    ...
                    dask_controller.futures[
                        ...
                        pixel_scaling,
                        metadata,
                        priority=random_priorities[k, k2] * 8,
                    )
    ...
    def extract_cell_data(
        ...
        segmented_mask_array = self.load_trench_array_list(
            self.phasesegmentationpath + "/segmentation_", file_idx, "data", False
        )
        # Load output file
        with HDFStore(
            ...
            for trench_idx in range(segmented_mask_array.shape[0]):
                for time_idx in range(segmented_mask_array.shape[1]):
                    ...
                    if len(seg_props) > 0:
                        ...
                        # Add metadata
                        seg_props["trenchid"] = [
                            global_trench_indices.loc[trench_idx, time_idx]
                        ] * len(seg_props["label"])
                        seg_props["file_trench_index"] = [trench_idx] * len(
                            seg_props["label"]
                        )
                        seg_props["time_s"] = [times.loc[trench_idx, time_idx]] * len(
                            seg_props["label"]
                        )
                        seg_props["trench_loadings"] = [
                            trench_loadings.loc[trench_idx, time_idx]
                        ] * len(seg_props["label"])
                        seg_props["fov"] = [fov_idx.loc[trench_idx, time_idx]] * len(
                            seg_props["label"]
                        )
                        if first_dict_flag:
    ...
    def extract_cell_data_mask(
        ...
        kymograph = self.load_trench_array_list(
            self.kymographpath + "/kymograph_", file_idx, self.seg_channel, False
        )
        # Load output file
        with HDFStore(
            os.path.join(self.phasedatapath, "data_%d.h5" % file_idx)
            ...
            for trench_idx in range(segmented_mask_array.shape[0]):
                for time_idx in range(segmented_mask_array.shape[1]):
                    # get regionprops
                    seg_props = sk.measure.regionprops(
                        segmented_mask_array[trench_idx, time_idx, :, :],
                        intensity_image=kymograph[trench_idx, time_idx, :, :],
                        cache=False,
                    )
                    if len(seg_props) > 0:
                        # Convert to dict
