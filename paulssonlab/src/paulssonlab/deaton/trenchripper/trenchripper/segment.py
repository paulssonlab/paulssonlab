...
class phase_segmentation:
    ...
    def detect_rough_trenches(self, img, show_plots=False):
        def fill_holes(img):
            ...
            img_filled = sk.morphology.reconstruction(
                seed, img, selem=strel, method="erosion"
            )
    ...
    def findCellsInTrenches(self, img, mask, sigma, window_size, niblack_k):
        ...
        if sigma > 0:
            img_smooth = sk.filters.gaussian(
                img, sigma=sigma, preserve_range=True, mode="reflect"
            )

        thresh_niblack = sk.filters.threshold_niblack(
            img_smooth, window_size=window_size, k=niblack_k
        )
    ...
    def findWatershedMaxima(self, img, mask):
        maxima = self.findCellsInTrenches(
            img,
            mask,
            self.maxima_smooth_sigma,
    ...
    def findWatershedMask(self, img, mask):
        img_mask = self.findCellsInTrenches(
            img,
            mask,
            self.init_smooth_sigma,
            self.init_niblack_window_size,
            self.init_niblack_k,
        )
    ...
    def extract_connected_components_phase(
        self, threshed, maxima, return_all=False, show_plots=False
    ):
        ...
        conn_comp = sk.morphology.remove_small_objects(
            conn_comp, min_size=self.min_cell_size
        )
    ...
    def segment(self, img, return_all=False, show_plots=False):
        ...
        conn_comp = self.extract_connected_components_phase(
            img_mask, maxima, show_plots=show_plots
        )
    ...
    def find_watershed_mask_future(self, trench_mask_and_median_future):
        return (
            self.findWatershedMask(
                trench_mask_and_median_future[1], trench_mask_and_median_future[0]
            ),
            trench_mask_and_median_future[1],
        )

    def find_watershed_maxima_future(self, watershed_mask_and_median_future):
        return (
            self.findWatershedMaxima(
                watershed_mask_and_median_future[1], watershed_mask_and_median_future[0]
            ),
            watershed_mask_and_median_future[1],
        )

    def find_connected_component_future(self, watershed_maxima_and_mask_future):
        return self.extract_connected_components_phase(
            watershed_maxima_and_mask_future[1], watershed_maxima_and_mask_future[0]
        )
    ...
    def find_trench_masks_and_median_filtered_list(self, trench_array_list):
        ...
        for tr in range(trench_array_list.shape[0]):
            for t in range(trench_array_list.shape[1]):
                trench_masks_list[tr, t, :, :] = self.detect_trenches(
                    trench_array_list[tr, t, :, :]
                )
                img_medians_list[tr, t, :, :] = self.medianFilter(
                    trench_array_list[tr, t, :, :]
                )
    ...
    def find_watershed_mask_list(self, trench_mask_and_median_filtered_list):
        ...
        for tr in range(img_array_list.shape[0]):
            for t in range(img_array_list.shape[1]):
                watershed_masks_list[tr, t, :, :] = self.findWatershedMask(
                    img_array_list[tr, t, :, :], trench_mask_array_list[tr, t, :, :]
                )
    ...
    def find_watershed_maxima_list(self, watershed_mask_and_median_filtered_list):
        ...
        for tr in range(img_array_list.shape[0]):
            for t in range(img_array_list.shape[1]):
                watershed_maxima_list[tr, t, :, :] = self.findWatershedMaxima(
                    img_array_list[tr, t, :, :], masks_list[tr, t, :, :]
                )
        return watershed_maxima_list, masks_list

    def find_conn_comp_list(
        self, watershed_maxima_and_masks_list
    ):  # masks_arr is tr,t,y,x
        ...
        for tr in range(masks_list.shape[0]):
            for t in range(masks_list.shape[1]):
                final_masks_list[tr, t, :, :] = self.extract_connected_components_phase(
                    masks_list[tr, t, :, :], maxima_list[tr, t, :, :]
                )
    ...
    def get_trench_loading_fraction(self, img, t_idx=-1, trench_idx=-1, file_idx=-1):
        try:
            ...
            raise Exception(
                "ValueError happened detecting trench at f%dtr%dt%d, %s"
                % (file_idx, trench_idx, t_idx, str(e))
            )
        try:
            loaded = (img < sk.filters.threshold_otsu(img)) * trench_mask
        except ValueError as e:
            raise Exception(
                "ValueError happened applying otsu threshold at f%dtr%dt%d, %s"
                % (file_idx, trench_idx, t_idx, str(e))
            )
    ...
    def measure_trench_loading(self, trench_array_list, file_idx=-1):
        trench_loadings = np.empty(
            (trench_array_list.shape[0], trench_array_list.shape[1])
        )
        for tr in range(trench_array_list.shape[0]):
            for t in range(trench_array_list.shape[1]):
                trench_loadings[tr, t] = self.get_trench_loading_fraction(
                    trench_array_list[tr, t, :, :],
                    t_idx=t,
                    trench_idx=tr,
                    file_idx=file_idx,
...
class fluo_segmentation:
    ...
    def find_mask(self, cell_local_mask, min_eigvals, edge_threshold, min_obj_size=30):
        ...
        composite_mask = sk.morphology.remove_small_objects(
            composite_mask, min_size=min_obj_size
        )
    ...
    def get_mid_threshold_arr(self, wrap_eig, edge_threshold_scaling=1.0, padding=3):
        edge_threshold_kymo = []
    ...
    def get_scores(
        ...
        threshold_percs = [
            1.0 + threshold_step_perc * step
            for step in range(-threshold_perc_num_steps, threshold_perc_num_steps + 1)
        ]
        edge_thresholds = [
            mid_threshold_arr * threshold_perc for threshold_perc in threshold_percs
        ]
        conv_weight_arrs = []
        for edge_threshold in edge_thresholds:
            composite_mask = self.find_mask(
                cell_mask, min_eigvals, edge_threshold, min_obj_size=min_obj_size
            )
    ...
    def segment(self, img_arr):  # img_arr is t,y,x
        ...
        min_eigvals = np.array(
            [
                ...
                for t in range(inverted.shape[0])
            ]
        )
        del inverted
        ...
        mid_threshold_arr = self.get_mid_threshold_arr(
            min_eigvals,
            edge_threshold_scaling=self.edge_threshold_scaling,
        ...
        convex_scores = self.get_scores(
            cell_mask,
            min_eigvals,
            mid_threshold_arr,
            threshold_step_perc=self.threshold_step_perc,
            threshold_perc_num_steps=self.threshold_perc_num_steps,
        ...
        final_mask = sk.morphology.remove_small_objects(
            final_mask, min_size=self.min_obj_size
        )
    ...
    def dask_segment(self, dask_controller):
        ...
        for k, file_idx in enumerate(file_list):
            ...
            future = dask_controller.daskclient.submit(
                self.generate_segmentation, file_idx, retries=1, priority=priority
            )
...
class phase_segmentation_cluster(phase_segmentation):
    ...
    def generate_segmentation(self, future_8bit):
        with worker_client() as client:
            ...
            future_trench_mask_and_median = client.map(
                self.get_trench_mask_and_median_filter,
                list(future_8bit.reshape(-1, original_shape[2], original_shape[3])),
                retries=1,
                priority=4 * priorities[1],
            )
            future_watershed_mask_and_median = client.map(
                self.find_watershed_mask_future,
                future_trench_mask_and_median,
                retries=1,
                priority=8 ** priorities[2],
            )
            future_watershed_maxima_and_mask = client.map(
                self.find_watershed_maxima_future,
                future_watershed_mask_and_median,
                retries=1,
                priority=16 * priorities[3],
            )
            future_final_mask = client.map(
                self.find_connected_component_future,
                future_watershed_maxima_and_mask,
                retries=1,
                priority=32 * priorities[4],
            )
            final_masks = np.array(client.gather(future_final_mask)).reshape(
                original_shape
            )
    ...
    def load_trench_array_list(self, path_form, file_idx, key, to_8bit):
        with h5py.File(path_form + str(file_idx) + ".hdf5", "r") as input_file:
            if to_8bit:
                ...
                for tr in range(trench_array_list.shape[0]):
                    for t in range(trench_array_list.shape[1]):
                        trench_array_list[tr, t, :, :] = self.to_8bit(
                            input_file[key][tr, t, :, :]
                        )
            else:
    ...
    def dask_segment(self, dask_controller):
        ...
        for k, file_idx in enumerate(file_list):
            future = dask_controller.daskclient.submit(
                self.load_trench_array_list,
                self.kymographpath + "/kymograph_",
                file_idx,
                self.seg_channel,
                True,
                retries=1,
            ...
            future = dask_controller.daskclient.submit(
                self.find_trench_masks_and_median_filtered_list,
                future,
                retries=1,
                priority=random_priorities[k, 1] * 0.4,
            )
            future = dask_controller.daskclient.submit(
                self.find_watershed_mask_list,
                future,
                retries=1,
                priority=random_priorities[k, 2] * 1.6,
            )
            future = dask_controller.daskclient.submit(
                self.find_watershed_maxima_list,
                future,
                retries=1,
                priority=random_priorities[k, 3] * 6.4,
            )
            future = dask_controller.daskclient.submit(
                self.find_conn_comp_list,
                future,
                retries=1,
                priority=random_priorities[k, 4] * 25.6,
            )
            future = dask_controller.daskclient.submit(
                self.save_masks_to_hdf,
                file_idx,
                future,
                retries=1,
                priority=random_priorities[k, 5] * 51.2,
            )
            dask_controller.futures["Segmentation: " + str(file_idx)] = future
    ...
    def dask_postprocess_trench_loading(self, dask_controller):
        trench_loadings = np.concatenate(
            dask_controller.daskclient.gather(
                dask_controller.futures["Trench Loading"]
            ),
            axis=0,
        )
    ...
    def get_seg_prop_dataframe(
        ...
        if len(seg_props) > 0:
            ...
            if "bbox" in props_to_grab:
                seg_props[["min_row", "min_col", "max_row", "max_col"]] = pd.DataFrame(
                    seg_props["bbox"].tolist(), index=seg_props.index
                )
    ...
    def dask_extract_cell_data(self, dask_controller, props_to_grab):
        ...
        for k, file_idx in enumerate(file_list):
            ...
            if segmented_masks_file in os.listdir(self.phasesegmentationpath):
                ...
                trench_future = dask_controller.daskclient.submit(
                    self.load_trench_array_list,
                ...
                dask_controller.futures[
                    "Cell Props %d: " % file_idx
                ] = dask_controller.daskclient.submit(
                    self.extract_cell_data,
                    file_idx,
                    trench_future,
                    times,
                    global_trench_indices,
                    trench_loadings,
                    props_to_grab,
                    metadata,
                    priority=random_priorities[k, 0],
    ...
    def extract_cell_data(
        ...
        with HDFStore(
            os.path.join(self.phasedatapath, "data_%d.h5" % file_idx)
        ) as store:
            if "/metrics" in store.keys():
                store.remove("/metrics")
            ...
            for trench_idx in range(segmented_mask_array.shape[0]):
                for time_idx in range(segmented_mask_array.shape[1]):
                    seg_props = sk.measure.regionprops(
                        segmented_mask_array[trench_idx, time_idx, :, :], cache=False
                    )
                    if len(seg_props) > 0:
                        seg_props = self.props_to_dict(seg_props, props_to_grab)
                        seg_props = pd.DataFrame(seg_props, columns=props_to_grab)
                        ...
                        if "centroid" in props_to_grab:
                            ...
                            seg_props = seg_props.drop("centroid", axis=1)
                        seg_props["trench_loadings"] = trench_loadings.loc[
                            trench_idx, time_idx
                        ]
                        seg_props["file_trench_index"] = trench_idx
                        seg_props["trenchid"] = global_trench_indices.loc[
                            trench_idx, time_idx
                        ]
                        seg_props["time_s"] = times.loc[trench_idx, time_idx]
                        seg_props = seg_props.set_index(
                            ["file_trench_index", "time_s", "trench_cell_index"]
                        )
                        if first_successful_dataframe:
                            store.put(
                                "metrics", seg_props, format="table", data_columns=True
                            )
                            if metadata is not None:
                                store.get_storer("metrics").attrs.metadata = metadata
