...
class mother_tracker:
    ...
    def get_mother_cell_growth_props(self, mother_data_frame):
        ...
        for i in range(len(loading_fractions) - 5):
            if np.all(
                ~(
                    (loading_fractions[i : i + 5] > 0.35)
                    * (loading_fractions[i : i + 5] < 0.6)
                )
            ):
                cutoff_index = i
        if cutoff_index < 5:
        ...
        instantaneous_growth_rate_area_smoothed = np.gradient(major_axis_length, times)[
            1:
        ]
        ...
        doubling_time_dataframe = pd.DataFrame(
            doubling_time, index=None, columns=["time_s", "doubling_time_s"]
        )
        doubling_time_smoothed_dataframe = pd.DataFrame(
            doubling_time_smoothed, index=None, columns=["time_s", "doubling_time_s"]
        )
        ...
        return (
            doubling_time_dataframe,
            doubling_time_smoothed_dataframe,
            growth_rate_dataframe,
        )
...
class mother_tracker_cluster(mother_tracker):
    ...
    def get_growth_props(self, file_idx):
        ...
        for trench in trenches:
            ...
            dt_df, dt_smoothed_df, growth_rate_df = self.get_mother_cell_growth_props(
                mother_df
            )
    ...
    def growth_props_cluster(self, daskcontroller):
        ...
        for file_idx in file_list:
            dask_controller.futures.append(
                dask_controller.daskclient.submit(self.get_growth_props, file_idx)
            )
