...
class scorefn:
    ...
    def fsize(self, sizeT, sizet, u_size=0.0, sig_size=0.2):
    ...
    def del_size(self, sizes):
        ...
        for t in range(len(sizes) - 1):
            sizescore, _ = self.fsize(
                sizes[t + 1], sizes[t], u_size=self.u_size, sig_size=self.sig_size
            )
    ...
    def del_half_size(self, sizes):
        ...
        for t in range(len(sizes) - 1):
            ...
            size_tf = sizes[t + 1]
            sizescore, _ = self.fsize(
                size_tf, size_ti, u_size=self.u_size, sig_size=self.sig_size
            )
            sizescores.append(sizescore.T)
        sizescores = np.array(sizescores)
        return sizescores
    ...
    def get_unions(self, centroids, sizes):
        ...
        for t in range(len(sizes)):
            ...
            for k in range(1, sizes[t].shape[0]):
                ...
                union_centroid = (
                    sizes[t][k - 1] * centroids[t][k - 1]
                    + sizes[t][k] * centroids[t][k]
                ) / union_size
    ...
    def compute_score_arrays(self, centroids, sizes, bboxes):
        ...
        for t in range(len(posij)):
            ...
            pad_f_ki = np.pad(f_ki, ((1, 1), (0, 0)), "constant", constant_values=0.0)
            ...
            f_ij[undefined_logsum] = 0.0
            f_ik[undefined_logsum] = 0.0
            f_ki[undefined_logsum[:-1]] = 0.0
            f_e[undefined_logsum] = 0.0
            ...
            ij_score = (-log_prob_ij) - 100.0
            ij_score[ij_score > 0] = 0.0
            ij_score[ij_score < -100.0] = -100.0
            Cij.append(ij_score)

            ik_score = (-log_prob_ik) - 100.0
            ik_score[ik_score > 0] = 0.0
            ...
            e_score = (-log_prob_e) - 100.0
            e_score[e_score > 0.0] = 0.0
            e_score[e_score < -100.0] = -100.0
            Ce.append(e_score)
    ...
    def compute_neighbor_stats(self, centroids, bboxes, sizes):
        ...
        for t in range(len(centroids) - 1):
            ...
            _, normsizediff = self.fsize(
                sizes[t + 1], sizes[t], u_size=self.u_size, sig_size=self.sig_size
            )

            pos_norm_del_l = copy.copy(norm_del_l)
            pos_norm_del_l[pos_norm_del_l < 0.0] = np.inf

            pos_norm_sizediff = copy.copy(normsizediff)
            pos_norm_sizediff[pos_norm_sizediff < 0.0] = np.inf

            neighbor_idx = np.argmin(pos_norm_del_l, axis=0)

            neighbor_dist = pos_norm_del_l[
                neighbor_idx, np.arange(pos_norm_del_l.shape[1])
            ].flatten()
            neighbor_size = pos_norm_sizediff[
                neighbor_idx, np.arange(pos_norm_sizediff.shape[1])
            ].flatten()
    ...
    def plot_neighbor_stats(self, ax_dist, ax_size, centroids, bboxes, sizes):
        neighbor_dists, neighbor_sizes = self.compute_neighbor_stats(
            centroids, bboxes, sizes
        )
        ...
        ax_dist.set_xlim(0.0, dist_avg + (3.0 * dist_std))
        ax_size.set_xlim(0.0, size_avg + (3.0 * size_std))
    ...
    def plot_segmentation(self, ax, kymo_arr, t0=0, tf=-1, cmap="coolwarm"):
        ...
        for t in range(t0, tf):
            padded_img = np.pad(
                kymo_arr[t][::-1],
                ((0, 0), (self.viewpadding, self.viewpadding)),
                "constant",
            )
            ax.imshow(padded_img, extent=[t * x_dim, (t + 1) * x_dim, 0, img_shape[1]])

    def plot_ij_scores(
        self, ax, kymo_arr, centroids, Cij, t0=0, tf=-1, cmap="coolwarm"
    ):
        ...
        for t in range(t0, tf):
            ...
            adjusted_centroids = (
                centroids[t]
                + t * np.array([x_dim, 0])
                + np.array([self.viewpadding, 0])
            )
            ax.scatter(
                adjusted_centroids[:, 0],
                adjusted_centroids[:, 1],
                c="r",
                s=100,
                zorder=10,
            )
            ...
            if t > t0:
                ...
                for i in range(N_detect_i):
                    ...
                    for j in range(N_detect_f):
                        ...
                        ax.plot(
                            [
                                centroid_i[0] + (t - 1) * x_dim + self.viewpadding,
                                centroid_f[0] + t * x_dim + self.viewpadding,
                            ],
                            [centroid_i[1], centroid_f[1]],
                            c=cmap_fn(color_score),
                            linewidth=3,
                        )
        adjusted_centroids = (
            centroids[tf] + tf * np.array([x_dim, 0]) + np.array([self.viewpadding, 0])
        )
        ax.scatter(adjusted_centroids[:, 0], adjusted_centroids[:, 1], c="r")

    def plot_ik_scores(
        self, ax, kymo_arr, centroids, Cik, t0=0, tf=-1, cmap="coolwarm"
    ):
        cmap_fn = cm.get_cmap(cmap, 100)
        img_shape = kymo_arr.shape[1:][::-1]
        x_dim = img_shape[0] + (self.viewpadding * 2)
        ...
        for t in range(t0, tf):
                ...
                for i in range(N_detect_i):
                    for j in range(N_detect_f - 1):
                        ...
                        ax.plot(
                            [
                                centroid_i[0] + (t - 1) * x_dim + self.viewpadding,
                                centroid_k1[0] + t * x_dim + self.viewpadding,
                            ],
                            [centroid_i[1], (centroid_k1[1] + centroid_k2[1]) / 2],
                            c=cmap_fn(color_score),
                            linewidth=3,
                        )
            ax.scatter(
            ...
            ax.scatter(
                adjusted_merged_centroids[:, 0],
                adjusted_merged_centroids[:, 1],
                c="white",
                s=100,
                zorder=10,
            )
        ...
        ax.scatter(
            adjusted_centroids[:, 0], adjusted_centroids[:, 1], c="r", s=100, zorder=10
        )
        ax.scatter(
            adjusted_merged_centroids[:, 0],
            adjusted_merged_centroids[:, 1],
    ...
    def plot_ki_scores(
        self, ax, kymo_arr, centroids, Cki, t0=0, tf=-1, cmap="coolwarm"
    ):
        cmap_fn = cm.get_cmap(cmap, 100)
        img_shape = kymo_arr.shape[1:][::-1]
        x_dim = img_shape[0] + (self.viewpadding * 2)
            ...
            if t > t0:
                ...
                for i in range(N_detect_i - 1):
                    for j in range(N_detect_f):
                        ...
                        ax.plot(
                            [
                                centroid_k1[0] + (t - 1) * x_dim + self.viewpadding,
                                centroid_i[0] + t * x_dim + self.viewpadding,
                            ],
                            [(centroid_k1[1] + centroid_k2[1]) / 2, centroid_i[1]],
                            c=cmap_fn(color_score),
                            linewidth=3,
                        )
    ...
    def plot_score_metrics(
        ...
        with h5py.File(
            self.segpath + "/segmentation_" + str(file_idx) + ".hdf5"
        ) as infile:
        ...
        centroids, sizes, bboxes, empty_trenches = get_segment_props(
            labeled_data, interpolate_empty_trenches=True
        )
        ...
        self.plot_ij_scores(
            axs[1], labeled_data, centroids, Cij, t0=t0, tf=tf, cmap="coolwarm"
        )
        axs[1].set_title("Movement Transition Score")
        self.plot_ik_scores(
            axs[2], labeled_data, centroids, Cik, t0=t0, tf=tf, cmap="coolwarm"
        )
        axs[2].set_title("Division Transition Score")
        self.plot_ki_scores(
            axs[3], labeled_data, centroids, Cki, t0=t0, tf=tf, cmap="coolwarm"
        )
    ...
    def interactive_scorefn(self):
        ...
        self.output = interactive(
            self.plot_score_metrics,
            {"manual": True},
            kymo_df=fixed(kymo_df),
            trenchid=IntSlider(value=0, min=0, max=num_trenches - 1, step=1),
            t_range=IntRangeSlider(
                value=[0, timepoints - 1],
                min=0,
                max=timepoints - 1,
                step=1,
                disabled=False,
            ...
            u_size=FloatSlider(value=self.u_size, min=0.0, max=1.0, step=0.01),
...
class tracking_solver:
    ...
    def define_tracking_problem(
        self, Cij, Cik, Cki, Ce, posij, posik, poski, sizes, merge_per_iter, edge_limit
    ):
        ...
        for t in range(len(Cij)):
            ...
            for i in range(cij.shape[0]):
                ...
                for j in range(cij.shape[1]):
                    if j >= i:
                        if j in valid_posij_slice:
                            Aij[(i, j, t)] = LpVariable(
                                "ai=" + str(i) + ",j=" + str(j) + ",t=" + str(t),
                                0,
                                1,
                                LpInteger,
                            )
                        ...
                        if j < (cij.shape[1] - 1):
                            ...
                            if j in valid_posik_slice:
                                Aik[(i, j, t)] = LpVariable(
                                    "ai=" + str(i) + ",k=" + str(j) + ",t=" + str(t),
                                    0,
                                    1,
                                    LpInteger,
                                )
                        ...
                        if i < (cij.shape[0] - 1):
                            ...
                            if j in valid_poski_slice:
                                Aki[(i, j, t)] = LpVariable(
                                    "ak=" + str(i) + ",i=" + str(j) + ",t=" + str(t),
                                    0,
                                    1,
                                    LpInteger,
                                )
                ...
                Ai[(i, t)] = LpVariable(
                    "ai=" + str(i) + ",t=" + str(t), 0, 1, LpInteger
                )
                obj_fn_terms.append(Ai[(i, t)])
                obj_fn_coeff.append(0.0)

                Ae[(i, t)] = LpVariable(
                    "aei=" + str(i) + ",t=" + str(t), 0, 1, LpInteger
                )
        ...
        prob += (
            sum(
                [
                    obj_fn_coeff[n] * obj_fn_term
                    for n, obj_fn_term in enumerate(obj_fn_terms)
                ]
            ),
            "Objective Function",
        )
        ...
        for t in range(0, len(sizes)):
            if t == 0:
                ...
                for j in range(N_detect):
                    neg_Aij = [
                        key for key in Aij.keys() if (key[0] == j) and (key[2] == t)
                    ]
                    neg_Aik = [
                        key for key in Aik.keys() if (key[0] == j) and (key[2] == t)
                    ]
                    neg_Aki = [
                        key for key in Aki.keys() if (key[0] == j - 1) and (key[2] == t)
                    ] + [key for key in Aki.keys() if (key[0] == j) and (key[2] == t)]
                    neg_Ai = Ai[(j, t)]
                    neg_Ae = Ae[(j, t)]

                    prob += (
                        sum([Aij[key] for key in neg_Aij])
                        + sum([Aik[key] for key in neg_Aik])
                        + sum([Aki[key] for key in neg_Aki])
                        + neg_Ai
                        + neg_Ae
                        <= 1,
                ...
                for j in range(N_detect_f):
                    ...
                    pos_Aik = [
                        key for key in Aik.keys() if key[1:] == (j - 1, t - 1)
                    ...
                    if j == N_detect_f - 1:
                        ...
                        prob += (
                            sum([Aij[key] for key in pos_Aij])
                            + sum([Aik[key] for key in pos_Aik])
                            + sum([Aki[key] for key in pos_Aki])
                            + sum([Ae[key] for key in pos_Ae])
                            - sum([Aij[key] for key in neg_Aij])
                            - sum([Aik[key] for key in neg_Aik])
                            - sum([Aki[key] for key in neg_Aki])
                            - neg_Ai
                        ...
                        prob += (
                            N_detect_f * neg_Ai
                            + sum([Aij[key] for key in above_Aij])
                            + sum([Aik[key] for key in above_Aik])
                            + sum([Aki[key] for key in above_Aki])
                            + sum([Ae[key] for key in above_Ae])
                            <= N_detect_f,
                        ...
                        prob += (
                            ...
                            + sum([Aik[key] for key in above_Aik])
                            + sum([Aki[key] for key in above_Aki])
                            + sum([Ai[key] for key in above_Ai])
                            <= N_detect_f,
        ...
        prob += (
            sum([value for _, value in Aki.items()]) <= merge_per_iter,
            "Merge Limit",
        )
    ...
    def get_sln_params(
        self, Aij, Aik, Aki, Ai, Ae, posij, posik, poski, sizes, edge_limit
    ):
        Aij_arr_list = [
            np.zeros((len(sizes[t]), len(sizes[t + 1])), dtype=bool)
            for t in range(len(sizes) - 1)
        ]
        Aik_arr_list = [
            np.zeros((len(sizes[t]), len(sizes[t + 1]) - 1), dtype=bool)
            for t in range(len(sizes) - 1)
        ]
        Aki_arr_list = [
            np.zeros((len(sizes[t]) - 1, len(sizes[t + 1])), dtype=bool)
            for t in range(len(sizes) - 1)
        ]
    ...
    def split_all_merged(self, labeled_data, Aki_arr_list):
        ...
        for t in range(len(Aki_arr_list)):
            if np.any(Aki_arr_list[t]):
                merged_label_list = (
                    np.where(np.any(Aki_arr_list[t], axis=1))[0] + 1
                ).tolist()
    ...
    def plot_tracking_sln(
        ...
        for t in range(t0, tf):
            ...
            ax.imshow(
                np.ma.array(padded_img, mask=(padded_img == 0)),
                extent=[t * x_dim, (t + 1) * x_dim, 0, img_shape[1]],
                cmap="Set1",
                vmin=0,
            )
            ...
            for idx in range(len(Aij_cords[0])):
                ...
                ax.plot(
                    [
                        centroid_i[0] + t * x_dim + self.viewpadding,
                        centroid_j[0] + (t + 1) * x_dim + self.viewpadding,
                    ],
            ...
            for idx in range(len(Aik_cords[0])):
                ...
                ax.plot(
                    [
                        centroid_i[0] + t * x_dim + self.viewpadding,
                        centroid_k2[0] + (t + 1) * x_dim + self.viewpadding,
                    ],
                    [centroid_i[1], centroid_k2[1]],
                    c="c",
                    linewidth=3,
                )
            ...
            for idx in range(len(Aki_cords[0])):
                ...
                ax.plot(
                    ...
                    [centroid_k1[1], centroid_i[1]],
                    c="c",
                    linewidth=3,
                )
                ax.plot(
                    [
                        centroid_k2[0] + t * x_dim + self.viewpadding,
                        centroid_i[0] + (t + 1) * x_dim + self.viewpadding,
                    ],
                    [centroid_k2[1], centroid_i[1]],
                    c="c",
                    linewidth=3,
                )
    ...
    def run_iteration(self, labeled_data, merge_per_iter, edge_limit):
        ...
        Cij, Cik, Cki, Ce, posij, posik, poski = self.ScoreFn.compute_score_arrays(
            centroids, sizes, bboxes
        )
        prob, Aij, Aik, Aki, Ai, Ae = self.define_tracking_problem(
            Cij, Cik, Cki, Ce, posij, posik, poski, sizes, merge_per_iter, edge_limit
        )
        prob.solve()
        (
            Aij_arr_list,
            ...
            Aij, Aik, Aki, Ai, Ae, posij, posik, poski, sizes, edge_limit
        )
    ...
    def compute_lineage(self, data, orientation):
        ...
        iter_outputs.append(
            self.run_iteration(
                working_labeled_data, self.merge_per_iter, self.edge_limit
            )
        )
        ...
        while merged_cells:
            working_labeled_data = self.split_all_merged(
                working_labeled_data, Aki_arr_list
            )
        ...
        (
            ...
            Ae_arr_list,
            empty_trenches,
        ) = self.run_iteration(working_labeled_data, 0, self.edge_limit)
        objective_val = value(prob.objective)
        active_edges = sum([v.varValue for v in prob.variables()])
        lineage_score = objective_val / active_edges

        return (
            working_labeled_data,
            centroids,
            sizes,
            Aij_arr_list,
            Aik_arr_list,
            ...
            if t not in empty_trenches:
                ...
                if self.intensity_channel_list is not None:
                    ...
                    for i, intensity_channel in enumerate(self.intensity_channel_list):
                        intensity_rps = sk.measure.regionprops(
                            labeled_data[t], kymo_arr_list[i][file_trench_idx, t]
                        )
    ...
    def lineage_trace(self, file_idx, file_trench_idx):
        ...
        fov_df = fov_df.set_index(
            ["File Index", "File Trench Index"], drop=True, append=False, inplace=False
        )
        ...
        mother_dict, daughter_dict, cell_ids_list = self.get_nuclear_lineage(
            sizes, empty_trenches, Aik_arr_list, Ae_arr_list
        )
    ...
    def lineage_trace_all_files(self, dask_cont):
        ...
        for k, file_idx in enumerate(file_list):
            ...
            future = dask_cont.daskclient.submit(
                self.lineage_trace_file, file_idx, retries=1, priority=priority
            )
    ...
    def compile_lineage_data(self, dask_cont):
        ...
        file_list = kymo_meta["File Index"].unique().tolist()
        num_file_jobs = len(file_list)
        file_idx_list = dask_cont.daskclient.gather(
            [
                dask_cont.futures["File Index: " + str(file_idx)]
