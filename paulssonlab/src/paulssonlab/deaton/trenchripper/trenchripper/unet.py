...
class data_augmentation:
    ...
    def deform_img_arr(self, img_arr, seg_arr):
        ...
        for t in range(img_arr.shape[0]):
            ...
            elastic_border = (
                map_coordinates(seg_arr[t, :, :] == 2, indices, order=1).reshape(
                    seg_arr.shape[1:3]
                )
                > 0.5
            )
...
class UNet_Training_DataLoader:
    ...
    def get_ttv_lists(self, ttv_split):
        ...
        trench_count_arr[
            trench_count_arr > self.num_trenches_per_fov
        ] = self.num_trenches_per_fov
                ...
                with h5py.File(img_path, "r") as imgfile:
                    with h5py.File(seg_path, "r") as segfile:
                        ...
                        for i, trenchid in enumerate(trenchids):
                            ...
                            img_handle[
                                (trench_count_accum[idx] * t_dim)
                                + i * t_dim : (trench_count_accum[idx] * t_dim)
                                + (i + 1) * t_dim
                            ] = img_arr
