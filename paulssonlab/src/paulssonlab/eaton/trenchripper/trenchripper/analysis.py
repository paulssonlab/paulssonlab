            ...
            for t in range(seg_arr.shape[1]):
                ...
                if self.intensity_channel_list is not None:
                    for i, intensity_channel in enumerate(self.intensity_channel_list):
                        ...
                        props_list = [
                            [file_idx, k, t, obj, intensity_channel]
                            + [getattr(rp, prop_key) for prop_key in self.props]
    ...
    def analyze_all_files(self, dask_cont):
        ...
        for k, file_idx in enumerate(file_list):
            ...
            future = dask_cont.daskclient.submit(
                self.get_file_regionprops, file_idx, retries=1, priority=priority
            )
...
class kymograph_viewer:
    ...
    def get_kymograph_data(self, file_idx, trench_idx):
        with h5py.File(
            self.kymopath + "/kymograph_" + str(file_idx) + ".hdf5", "r"
        ) as infile:
            kymodat = infile[self.channel][trench_idx]
        with h5py.File(
            self.segpath + "/segmentation_" + str(file_idx) + ".hdf5", "r"
        ) as infile:
            segdat = infile["data"][trench_idx]
        segdat = np.array(
            [
                sk.morphology.label(segdat[t], connectivity=2)
                for t in range(segdat.shape[0])
            ]
        )
