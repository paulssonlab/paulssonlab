...
class fluo_segmentation:
    ...
    def cell_region_mask(
        ...
        if method == "global":
            ...
            cell_region_threshold = (
                sk.filters.threshold_otsu(local_otsu) * cell_otsu_scaling
            )
    ...
    def segment(self, img_arr):
        ...
        cell_mask = self.cell_region_mask(
            ...
            global_otsu_scaling=self.global_otsu_scaling,
...
class fluo_segmentation_cluster(fluo_segmentation):
    def __init__(
        ...
        super(fluo_segmentation_cluster, self).__init__(
            ...
            global_otsu_scaling=global_otsu_scaling,
    ...
    def init_fov_number(self, fov_number):
        ...
        self.input_file_path = (
            self.input_path + "/kymo_" + str(self.fov_number) + ".hdf5"
        )
            ...
            for trenchid in input_file.keys():
                ...
                with h5py.File(self.output_file_path, "a") as h5pyfile:
                    hdf5_dataset = h5pyfile.create_dataset(
                        str(trenchid), data=trench_array, dtype=bool
                    )
            return "Done"
    ...
    def dask_segment(self, dask_controller, fov_list=None):
        ...
        trenchid_dict = {
            fov: df_in.loc[fov]["trenchid"].unique().tolist() for fov in fov_list
        }
        ...
        for fov in fov_list:
            for trenchid in trenchid_dict[fov]:
                future = dask_controller.daskclient.submit(
                    seg, fov, trenchid, retries=2
                )
                dask_controller.futures[
                    "generate_segmentation: " + str(fov) + "," + str(trenchid)
                ] = future
...
class phase_segmentation:
    ...
    def cell_region_mask(
        ...
        for t in range(t_tot):
            ...
            cell_threshold = (
                sk.filters.threshold_otsu(trench_values) * global_otsu_scaling
            )
            cell_thr_arr = cell_threshold * np.ones(
                wrap_img[:, :, t].shape, dtype="uint8"
            )
    ...
    def get_mid_threshold_arr(
        self, wrap_eig, wrap_trench_mask, edge_threshold_scaling=1.0, padding=3
    ):
        edge_threshold_kymo = []
        for t in range(wrap_eig.shape[-1]):
    ...
    def segment(self, img_arr):
        ...
        del input_kymo
        trench_mask = working_img > (
            sk.filters.threshold_triangle(working_img) * self.trench_mask_scaling
        )
        ...
        min_eigvals = self.to_8bit(
            self.hessian_contrast_enc(working_img, self.hess_pad)
        )
...
class phase_segmentation_cluster(phase_segmentation):
    def __init__(
        ...
        super(phase_segmentation_cluster, self).__init__(
            ...
            threshold_perc_num_steps=threshold_perc_num_steps,
            convex_threshold=convex_threshold,
            trench_mask_scaling=trench_mask_scaling,
        )
            ...
            with h5py.File(self.output_file_path, "w") as h5pyfile:
                for trenchid in input_file.keys():
                    ...
                    try:
                        ...
                        hdf5_dataset = h5pyfile.create_dataset(
                            str(trenchid), data=out_array, dtype=bool
                        )
                    except:
