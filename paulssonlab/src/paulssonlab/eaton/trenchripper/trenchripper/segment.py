...
class fluo_segmentation:
    ...
    def cell_region_mask(
        self, img_arr, method="global", cell_otsu_scaling=1.0, t_tot=0, local_otsu_r=15
    ):
        if method == "global":  ###Maybe combinw with mid_threshold class
            ...
            for t in range(t_tot):
                cell_threshold = (
                    sk.filters.threshold_otsu(wrap_img[:, :, t]) * cell_otsu_scaling
                )
    ...
    def segment(self, img_arr):
        ...
        convex_scores = self.get_scores(
            ...
            mjr_axis_range=self.mjr_axis_range,
            mnr_axis_range=self.mnr_axis_range,
        )
        del cell_mask
        del min_eigvals
            ...
            for lane in input_file.keys():
                ...
                with h5py.File(self.output_file_path, "w") as h5pyfile:
                    for trench in range(lane_array.shape[0]):
                        trenchid = kymo_meta.loc[
                            int(fov_number), int(lane), int(trench), 0
                        ]["trenchid"]
                        hdf5_dataset = h5pyfile.create_dataset(
                            str(trenchid), lane_array.shape[1:], dtype=bool
                        )
