...
class data_augmentation:
    ...
    def add_padding(self, img_arr, seg_arr, max_rot=20, min_padding=20):
        ...
        padded_seg_arr = np.pad(
            seg_arr,
            ((0, 0), (delta_y // 2, delta_y // 2), (delta_x // 2, delta_x // 2)),
            "constant",
            constant_values=0,
        )
    ...
    def translate(self, pad_img_arr, pad_seg_arr, img_arr, seg_arr):
        ...
        for t in range(pad_img_arr.shape[0]):
            ...
            trans_img_arr[
                t,
                0,
                delta_y // 2 : delta_y // 2 + img_arr.shape[0],
                delta_x // 2 : delta_x // 2 + img_arr.shape[1],
            ] = 0
            trans_seg_arr[
                t,
                delta_y // 2 : delta_y // 2 + img_arr.shape[0],
            ...
            trans_img_arr[
                t,
                0,
                delta_y // 2 + trans_y : delta_y // 2 + img_arr.shape[0] + trans_y,
                delta_x // 2 + trans_x : delta_x // 2 + img_arr.shape[1] + trans_x,
            ] = pad_img_arr[
                t,
                0,
                delta_y // 2 : delta_y // 2 + img_arr.shape[0],
    ...
    def rotate(self, img_arr, seg_arr, max_rot=20):
        ...
        for t in range(img_arr.shape[0]):
            ...
            rot_seg = (sk.transform.rotate(seg_arr[t, :, :] == 1, r) > 0.5).astype(
                "int8"
            )
    ...
    def deform_img_arr(self, img_arr, seg_arr):
        ...
        for t in range(img_arr.shape[0]):
            ...
            elastic_cell = (
                map_coordinates(seg_arr[t, :, :] == 1, indices, order=1).reshape(
                    seg_arr.shape[1:3]
                )
                > 0.5
            )
...
class UNet_Training_DataLoader:
    ...
    def get_metadata(self, headpath):
        ...
        trench_dict = {
            fov: len(
                kymo_handle.loc[fov].index.get_level_values("trench").unique().values
            )
            for fov in fov_list
        }
    ...
    def get_selection(
        ...
        trench_count = np.array(
            [min(max_trench_per_fov, trench_dict[fov]) for fov in fov_list]
        )
    ...
    def inter_get_selection(self, headpath, selectionname):
        ...
        self.selection = ipyw.interactive(
            self.get_selection,
            {"manual": True},
            ...
            max_trench_per_fov=ipyw.IntText(
    ...
    def export_data(self, selectionname):
        ...
        with h5py.File(nnpath, "w") as outfile:
            ...
            img_handle = outfile.create_dataset(
                "img", out_shape, chunks=chunk_shape, dtype="int32"
            )
            seg_handle = outfile.create_dataset(
                "seg", out_shape, chunks=chunk_shape, dtype="int8"
            )
            ...
            for idx, fov in enumerate(selection[1]):
                ...
                with h5py.File(img_path, "r") as imgfile, h5py.File(
                    seg_path, "r"
                ) as segfile:
                    ...
                    for i, trenchid in enumerate(trenchids):
                        img_arr = imgfile[trenchid + "/" + selection[0]][
                            :, :, selection[3][0] : selection[3][1] : selection[2]
                        ]
...
class UNet_Trainer:
    def __init__(
        ...
        if augment:
            self.data_augmentor = data_augmentation(
                p_flip=p_flip, max_rot=max_rot, min_padding=min_padding
            )
    ...
    def perepoch(self, e, train_iter, val_iter, train_data_shape, val_data_shape):
        ...
        for i, b in enumerate(train_iter):
            ...
            seg_arr = seg_arr[
                :, 0
            ]  # dirty fix move to data prep when scratch stabalizes
            if self.augment:
                img_arr, seg_arr = self.data_augmentor.get_augmented_data(
                    img_arr, seg_arr
                )
            weightmaps = []
            ...
            if self.writetotb:
                self.writer.add_scalar(
                    "Mean Train NLL", mean_nll, i + e * num_train_batches
                )
            if i % 25 == 0:
    ...
    def train_model(self, numepochs, train_data, val_data):
        ...
        for e in range(0, numepochs):
            ...
            val_iter = DataLoader(
                val_data, batch_size=self.batch_size, shuffle=True, num_workers=4
            )
    ...
    def get_test_pr(self, test_data, samples=1000):
        ...
        test_iter = DataLoader(
            test_data, batch_size=samples, shuffle=True, num_workers=4
        )
