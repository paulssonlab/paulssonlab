...
class Nd2(object):
    ...
    def __repr__(self):
        return "\n".join(
            [
                ...
                "Channels: %s"
                % ", ".join(["%s" % str(channel) for channel in self.channels]),
    ...
    def select(
        self, fields_of_view=None, channels=None, z_levels=None, start=0, stop=None
    ):
        """
        ...
        for frame in range(start, stop):
            (
                ...
                channel,
                z_level,
            ) = self._parser.driver.calculate_image_properties(frame)
            if (
                field_of_view in fields_of_view
                and channel in channels
                and z_level in z_levels
            ):
    ...
    def get_image(self, frame_number, field_of_view, channel_name, z_level):
        ...
        """
        return self._parser.driver.get_image_by_attributes(
            frame_number, field_of_view, channel_name, z_level, self.height, self.width
        )

    ### Added by Andrew Martens, May 2020
    def get_image_stack(self, frame_number, field_of_view, z_level):
        return self._parser.driver.get_image_stack(
            frame_number, field_of_view, z_level, self.height, self.width
        )
