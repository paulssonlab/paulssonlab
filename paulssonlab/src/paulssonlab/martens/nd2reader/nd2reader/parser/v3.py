...
class V3RawMetadata(object):
    ...
    def image_metadata_sequence(self):
        return read_metadata(
            read_chunk(self._fh, self._label_map.image_metadata_sequence), 1
        )
    ...
    def acquisition_times(self):
        acquisition_times = read_array(
            self._fh, "double", self._label_map.acquisition_times
        )
    ...
    def image_metadata(self):
        if self._label_map.image_metadata:
            return read_metadata(
                read_chunk(self._fh, self._label_map.image_metadata), 1
            )
...
class V3Parser(BaseParser):
    ...
    def _parse_metadata(self):
        ...
        """
        height = self.raw_metadata.image_attributes[six.b("SLxImageAttributes")][
            six.b("uiHeight")
        ]
        width = self.raw_metadata.image_attributes[six.b("SLxImageAttributes")][
            six.b("uiWidth")
        ]
        ...
        pixel_microns = self.raw_metadata.image_calibration.get(
            six.b("SLxCalibration"), {}
    ...
    def _parse_channels(self, raw_metadata):
        ...
        """
        channels = []
        metadata = raw_metadata.image_metadata_sequence[six.b("SLxPictureMetadata")][
            six.b("sPicturePlanes")
        ]
        try:
            validity = raw_metadata.image_metadata[six.b("SLxExperiment")][
                six.b("ppNextLevelEx")
            ][six.b("")][0][six.b("ppNextLevelEx")][six.b("")][0][six.b("pItemValid")]
        ...
        for (label, chan), valid in zip(
            sorted(metadata[six.b("sPlaneNew")].items()), validity
        ):
            if not valid:
                continue
    ...
    def _parse_total_images_per_channel(self, raw_metadata):
        ...
        """
        return raw_metadata.image_attributes[six.b("SLxImageAttributes")][
            six.b("uiSequenceCount")
        ]
