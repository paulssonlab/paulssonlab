...
from properties import write_properties_to_table, merge_tables, get_metadata, make_cell_type, subtract_background_from_coords
...
def run_segmentation_analysis_regions(in_file, name, fov, frame, z_level, seg_params, channels, out_dir_masks, out_dir_tables, algo_dict, file_names, regions_file):
    """
    ...
    for n in h5file_reg.iter_nodes("/{}/FOV_{}/Frame_{}/{}".format(name, fov, frame, z_level)):
        ...
        z_node = h5file.get_node("/Images/{}/FOV_{}/Frame_{}/Z_{}".format(name, fov, frame, z_level))
        (stack, ch_to_index) = make_ch_to_img_stack_regions(h5file, z_node, channels, regions)
        h5file.close()
        
        run_segmentation_analysis(in_file, name, fov, frame, z_level, seg_params, channels, out_dir_masks, out_dir_tables, algo_dict, file_names, region_set_number)
    
    h5file_reg.close()
    
def run_segmentation_analysis_no_regions(in_file, name, fov, frame, z_level, seg_params, channels, out_dir_masks, out_dir_tables, algo_dict, file_names):
    """
    ...
    run_segmentation_analysis(in_file, name, fov, frame, z_level, seg_params, channels, out_dir_masks, out_dir_tables, algo_dict, file_names, 0, stack, ch_to_index)
    
def run_segmentation_analysis(in_file, name, fov, frame, z_level, seg_params, channels, out_dir_masks, out_dir_tables, algo_dict, file_names, region_set_number, stack, ch_to_index):
    """
    ...
    pathlib.Path("{}/{}/FOV_{}/".format(out_dir_masks, name, fov)).mkdir(parents=True, exist_ok=False)
    h5file_masks = tables.open_file("{}/{}/FOV_{}/Frame_{}.h5".format(out_dir_masks, name, fov, frame), mode="a")
    
    # Create directory structure to store the tables
    pathlib.Path("{}/{}/FOV_{}/".format(out_dir_tables, name, fov)).mkdir(parents=True, exist_ok=False)
    h5file_tables = tables.open_file("{}/{}/FOV_{}/Frame_{}.h5".format(out_dir_tables, name, fov, frame), mode="a")
    ...
    write_masks_tables(h5file_masks, h5file_tables, name, fov, frame, z_level, table.row, seg_params, stack, ch_to_index, algo_dict, region_set_number)
...
def write_masks_tables(h5file_masks, h5file_tables, name, fov, frame, z_level, row, seg_params, stack, ch_to_img, algo_dict, region_set_number):
    """
    ...
    for sc in seg_params.keys():
        ...
        for region_number in range(masks.shape[2]):
            ...
            for p in properties:
                write_properties_to_table(name, fov, frame, z_level, region_set_number, region_number, p, sc, seg_params[sc], row, stack, ch_to_img)
...
def make_ch_to_img_stack_regions(h5file, z_node, channels, regions):
    ...
    stack = numpy.empty((x_dimension, y_dimension, regions.shape[0], len(channels)), dtype=numpy.float32, order='F')
...
def make_ch_to_img_stack_no_regions(h5file, z_node, channels):
    ...
    # NOTE Does copy flag actually make a difference?
    zeroth_img = h5file.get_node(z_node, zeroth_channel).read().astype(numpy.float32, casting="safe", copy=False)
    
    # Initalize the empty ndarray
    stack = numpy.empty((zeroth_img.shape[0], zeroth_img.shape[1], 1, len(channels)), dtype=numpy.float32, order='F')
    ...
    for i, ch in iter_channels::
        ...
        # NOTE Does copy flag actually make a difference?
        image = h5file.get_node(z_node, ch).read().astype(numpy.float32, casting="safe", copy=False)
...
def link_files(in_dir, file_name):
    ...
    for nd2_entry in os.scandir(in_dir):
        if nd2_entry.is_dir():
            ...
            h5file_nd2file = tables.open_file(nd2_h5_outfile, mode="w", title=nd2_entry.name)
            
            # FOV directories
            for fov_entry in os.scandir(nd2_entry):
                if fov_entry.is_dir():
                    h5file_fov = tables.open_file("{}.h5".format(fov_entry.path), mode="w", title=fov_entry.name)
                    ...
                    for frame_entry in os.scandir(fov_entry):
                        if frame_entry.is_file():
                            (name, extension) = os.path.splitext(os.path.basename(frame_entry.name))
                            if (extension == ".h5"):
            ...
            h5file_top.create_external_link("/{}".format(file_name),
...
def main_segmentation_function(out_dir, in_file, num_cpu, params_file, regions_file):
    ...
    with Pool(processes=num_cpu) as pool:
        ...
        algo_to_func = { 'threshold'         : algorithms.run_single_threshold,
        ...
        for n in h5file.iter_nodes(h5file.root.Images):
            ...
            total += len(metadata['fields_of_view']) * len(metadata['frames']) * len(metadata['z_levels'])
                ...
                for fov in metadata['fields_of_view']:
                    for frame in metadata['frames']:
                        for z_level in metadata['z_levels']:
                            func_args = [in_file, n._v_name, fov, frame, z_level, params, channels, out_dir_masks, out_dir_tables, algo_dict, file_names, regions_file]
                            
                            pool.apply_async(algorithms.run_segmentation_analysis_regions, func_args, callback=update_pbar)
                ...
                for fov in metadata['fields_of_view']:
                    for frame in metadata['frames']:
                        for z_level in metadata['z_levels']:
                            ...
                            func_args = [in_file, n._v_name, fov, frame, z_level, params, channels, out_dir_masks, out_dir_tables, algo_dict, regions_file, file_names]
                            
                            pool.apply_async(algorithms.run_segmentation_analysis_no_regions, func_args, callback=update_pbar)
                            #func(*func_args) # DEBUG
