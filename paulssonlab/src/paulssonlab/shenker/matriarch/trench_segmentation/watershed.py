...
def segment_trench(img, threshold=4e-2, diagnostics=None):
    ...
    bg_label = scipy.stats.mode(labeled_img.flat[labeled_img.flat != 0]).mode[0]
    if diagnostics is not None:
        diagnostics["bg_label"] = bg_label
    ...
    labeled_distances = skimage.morphology.label(
        distances != 0 + (labeled_img == bg_label)
    )
    if diagnostics is not None:
        diagnostics["labeled_distances"] = _trench_img(labeled_distances)
    bg_label2 = scipy.stats.mode(labeled_distances.flat != 0).mode[0]
