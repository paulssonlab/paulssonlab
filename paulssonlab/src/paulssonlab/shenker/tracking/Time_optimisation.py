...
def main2(mainhuh):
    ...
    for mn in range(len(arr_m)):
        ...
        img_ini = cv2.imread("%d.jpg" % (mn + 10))
        th = [[]] * 4
        imgs = [[]] * 4
        ...
        if mn > 0:
            ...
            for i in range(1, n_constr[mn] + 1):
                # con1 = model_tree.addConstr(s[mn][(tree2(0,3)[int(np.where([x[0]==i for x in tree2(0,3)])[0])][1])-1]+s[mn][(n_constr_b[mn])+(tree2(1,3)[int(np.where([x[0]==i for x in tree2(1,3)])[0])][1])]+s[mn][n_constr_a[mn]+(tree2(2,3)[int(np.where([x[0]==i for x in tree2(2,3)])[0])][1])] +s[mn][n_constr_c[mn]+i] == 1)
    ...
    concatenated2.save("time_img.jpg")
