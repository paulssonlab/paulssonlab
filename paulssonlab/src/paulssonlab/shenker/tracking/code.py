...
def main2(mainhuh):
    ...
    def get_center(threshold_number, mask_number):
        ...
        right_edge = max_x[1]
        left_edge = min_x[1]
        # print(arr_center_x,  pp, len_x, right_edge, left_edge)
    ...
    for mn in range(len(arr_m)):
        ...
        img_ini = cv2.imread("%d.jpg" % (mn))  # +44))#+20+14+2-36))#+29+2)) #12
        ...
        for j in range(len(df_list[mn - 1])):
            ...
            for i in range(len(df_list[mn])):  ###
                # 0.156#0.026#####0.226
                cost_move[j][i] = (
                    14.226 * (abs(center_arr[i] - df_list[mn - 1]["center"].values[j]))
                    + 0.050 * abs(peaks[i] + df_list[mn - 1]["peaks"].values[j] - 2)
                    + +(0.001 * df_list[mn - 1]["length_x"].values[j])
                    + 0.026 * abs(lenx_arr[i] - df_list[mn - 1]["length_x"].values[j])
                )

                if (
                    lenx_arr[i] < 5 or df_list[mn - 1]["length_x"].values[j] < 5
        ...
        if mn > 0:
                ...
                for i in range(len(df_list[mn])):
                    ...
                    if c_i[i] != max(center_arr):
                        ...
                        for j in index_i:  #
                            ...
                            if (
                                df_list[mn]["length_x"].values[i] < 5
                                or df_list[mn - 1]["length_x"].values[k] < 5
                                or df_list[mn]["length_x"].values[j] < 5
                                ...
                                cost_divide[k, i, j] = cost_divide[k, i, j] + 10000
                            else:
            ...
            for k in range(0, len(df_list[mn - 1])):
                x_exit = 0
                if df_list[mn - 1]["length_x"].values[k] < 5 or len(df_list[mn]) >= len(
                    df_list[mn - 1]
                ):
                    x_exit = 100000
        ...
        if mn > 0:
            ...
            for i in range(len(df_list[mn - 1])):
                ...
                if c_a[i] < max(df_list[mn - 1]["left_edge"]):
                    ...
                    # print(c_a[i], type(index_ia[i][0]), index_ia[i][0])
                    # sum_see2[i] =  sum(quicksum(a_div3[mn-1][ia][j][k] for j in range (0,lmn) for k in range (0,lmn) if type(a_div3[mn-1][ia][j][k] ) != int  )  + quicksum(a_div2[mn-1][ia][k] for k in range (0,lmn) if type(a_div2[mn-1][ia][k] ) != int)  for ia in index_ia[i])

                    sum_see2[i] = quicksum(
                        (s[mn - 1][kl] - a_exit[mn - 1][kl]) for kl in index_ia[i]
                    )
                    # if (type(sum_see2[i]) != int):
                    k = len(df_list[mn - 1])
    ...
    for ik in range(len(asg)):
        ...
        # print(s_asg_arr[ik].split('_'))
        asg["int"][ik] = spl[2]
        asg["t"][ik] = spl[1]
        # asg['split'] = s_asg_arr[i].split('_')
    ...
    """for i in range(len(arr_m)):
        for k in range(len(df_list[i])):
            if type(sum_see3[i][k]) != int:
                print(i,k , df_list[2]['left_edge'][16], LinExpr.getValue(sum_see3[i][k]))"""
    ...
    video = cv2.VideoWriter(
        "video_mask_1abcc.mp4", -1, 5, (width, height)
    ...
    concatenated2.save("mask_img_e8h.jpg")


img_out = main2(65)  # 9)#31+34)#18
