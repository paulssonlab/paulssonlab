...
t = 7
list2 = [2, 0, 1, -2, -1]
pixelsize_list = [140]  # 100,50, 80, 30, 40]
bright_halo_list = ["rgb(68,68,68)", "rgb(48,48,48)", "rgb(88,88,88)"]
bright_blur_list = ["rgb(0,0,0)", "rgb(60,60,60)", "rgb(40,40,40)"]
mean_brightness = 225
sigma_brightness = 0  # 10
brightnesses = [np.random.normal(mean_brightness, sigma_brightness) for i in range(4)]

bright_list = [
    "rgb(%d,%d,%d)"
    % (int(brightnesses[0]), int(brightnesses[0]), int(brightnesses[0])),
    "rgb(%d,%d,%d)"
    % (int(brightnesses[1]), int(brightnesses[1]), int(brightnesses[1])),
    "rgb(%d,%d,%d)"
    % (int(brightnesses[2]), int(brightnesses[2]), int(brightnesses[2])),
    "rgb(%d,%d,%d)"
    % (int(brightnesses[3]), int(brightnesses[3]), int(brightnesses[3])),
]


print(">", mean_brightness, sigma_brightness, bright_list)
...
# m1_mat[0] = np.array([179,158])#,149,169,160,124,106])

m1_mat[0] = np.array([179, 158, 140])  # ,149,169,160,124,106])
...
for i in range(1, t):
    ...
    for j in range(len(m1_mat[i - 1])):
        if m1_mat[i - 1][j] <= 180:
            m1_mat[i][j] = int(m1_mat[i - 1][j] + random.uniform(10, 20))  # 10,20
    ...
    df_2["center"][i - 1] = [0] * len(m1_mat[i - 1])
    ...
    for j in range(len(m1_mat[i - 1])):
        ...
        df_2["center"][i - 1][j] = (df["center"][i - 1][j] * 1.4) + 135
...
for i in range(0, t - 1):
    ...
    for j in range(len(df["to"][i])):
        # print('>',i, j , df['to'][i][j])
        df_2["to"][i][j] = (df["to"][i][j] * 1.4) + 135
        # print(df_2['to'][i][j])

    df_2["to"][i] = np.array(df_2["to"][i])  # [df_2['to'][i]!=None] )
    df_2["to"][i] = df_2["to"][i][df_2["to"][i] != None]
    print(df_2["to"][i])
    df["to_length"][i] = df["len_mat"][i + 1]

# df_2['to'][t-1] = np.array(df_2['to'][t-1][df_2['to'][t-1]!=None] )
...
video = cv2.VideoWriter(
    "video_intensity_1.mp4", -1, 5, (width, height)
...
concatenated2.save("mask_img5.jpg")

df = df.drop(columns=["from", "to_length", "re2"])
df.to_csv("df2.csv")

df = df.drop(columns=["t", "len_mat", "right_edge"])

df1 = df.iloc[:-2]
df1.to_csv("ground_truth5.csv")
df_2.to_csv("ground_truth_b.csv")
