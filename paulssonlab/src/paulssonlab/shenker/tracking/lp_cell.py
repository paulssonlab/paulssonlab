...
def main2(mainhuh):
    ...
    for mn in range(len(arr_m)):
        ...
        cost_divide = [[[0] * len(df_list[mn])] * len(df_list[mn])] * len(
            df_list[mn - 1]
        )
        for k in range(len(df_list[mn - 1])):
            ...
            for i in range(len(df_list[mn])):
                ...
                for j in range(len(df_list[mn])):
                    cost_divide[k][i][j] = (
                        20
                        * (lenx_arr[i] + lenx_arr[j] - df_list[mn - 1]["length_x"][k])
                        + 300 * abs(center_arr[i] - (df_list[mn - 1]["center"][k]))
                        + 100 * abs(center_arr[j] - (center_arr[i]))
                        + 50
                        * abs(
                            center_arr[j]
                            + (center_arr[i] - 2 * (df_list[mn - 1]["center"][k]))
                        )
                    )
                    xm = model_tree.addVar(
                        0,
        ...
        for i in range(1, n_constr[mn] + 1):
            con1 = model_tree.addConstr(
                s[mn][
                    (tree2(0, 3)[int(np.where([x[0] == i for x in tree2(0, 3)])[0])][1])
                    - 1
                ]
                + s[mn][
                    (n_constr_b[mn])
                    + (
                        tree2(1, 3)[int(np.where([x[0] == i for x in tree2(1, 3)])[0])][
                            1
                        ]
                    )
                ]
                + s[mn][
                    n_constr_a[mn]
                    + (
                        tree2(2, 3)[int(np.where([x[0] == i for x in tree2(2, 3)])[0])][
                            1
                        ]
                    )
                ]
            ...
            if type(cen_whi[i]) != int and cen_whi[i] != 0:  ##############
                for j in range(0, len(cen_whi[i])):
                    if (df_list_concat["t"][k] == df_list_concat["t"][i] + 1) and (
                        df_list_concat["index"][k] == int(cen_whi[i][j])
                    ):
                        cen_val[i] += [str((df_list_concat["center"][k]))]
    ...
    concatenated2.save("test_ichecking.jpg")
